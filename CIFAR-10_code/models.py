...
class CondResBlock(nn.Module):
    def __init__(self, downsample=True, rescale=True, filters=64, latent_dim=64, im_size=64, classes=512, norm=True, spec_norm=False):
...
class MNISTModel(nn.Module):
    ...
    def init_main_model(self):
        ...
        self.res1 = CondResBlock(filters=filter_dim, latent_dim=1, im_size=im_size)
        self.res2 = CondResBlock(filters=2*filter_dim, latent_dim=1, im_size=im_size)

        self.res3 = CondResBlock(filters=4*filter_dim, latent_dim=1, im_size=im_size)
...
class ResNetModel(nn.Module):
    ...
    def init_main_model(self):
        ...
        self.res_1a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.res_1b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.res_2a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.res_2b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.res_3a = CondResBlock(filters=2*filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.res_3b = CondResBlock(filters=2*filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.res_4a = CondResBlock(filters=4*filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.res_4b = CondResBlock(filters=4*filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)
    ...
    def init_mid_model(self):
        ...
        self.mid_res_1a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.mid_res_1b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.mid_res_2a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.mid_res_2b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.mid_res_3a = CondResBlock(filters=2*filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.mid_res_3b = CondResBlock(filters=2*filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)
    ...
    def init_small_model(self):
        ...
        self.small_res_1a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.small_res_1b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.small_res_2a = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, downsample=False, spec_norm=self.spec_norm, norm=self.norm, classes=2)
        self.small_res_2b = CondResBlock(filters=filter_dim, latent_dim=latent_dim, im_size=im_size, rescale=True, spec_norm=self.spec_norm, norm=self.norm, classes=2)

        self.small_energy_map = nn.Linear(filter_dim*2, 1)

    def main_model(self, x, latent = None, compute_feat=False):
        x = self.act(self.conv1(x))
    ...
    def forward(self, x, latent = None):
        ...
        if args.multiscale:
            ...
            energy = torch.cat([small_energy, mid_energy, large_energy], dim=-1).mean()
...
class CelebAModel(nn.Module):
    ...
    def forward(self, x, latent):
        ...
        energy = self.small_model(x, latent)
