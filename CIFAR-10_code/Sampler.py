...
class Sampler:
    ...
    def sample_new_exmps(self, steps, step_size,noise_level,batch_size,num_batch):
        ...
        for i in range(num_batch):
            ...
            # Construct a batch 
            batch = torch.multinomial(torch.ones((self.sample_size,)),batch_size,replacement = False)
            ...
            self.log_weights = self.log_weights+(-new_energy + self.old_energy).squeeze()
        ...
        return self.examples[batch].detach(),self.log_weights[batch].detach()
    ...
    def generate_samples(self, model, inp_imgs, steps, step_size, noise_level, return_img_per_step=False):
        ...
        for _ in range(steps):
            ...
            inp_imgs.grad.data.clamp_(-0.3/step_size, 0.3/step_size) # For stabilizing and preventing too high gradients
        ...
        # Re-activate gradients for parameters for training
        for p in model.parameters():
    ...
    def resample_multinomial(self):
        ...
        new_index = torch.multinomial(self.weights,self.sample_size,replacement = True)
